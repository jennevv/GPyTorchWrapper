...
from ..utils.input_transformer import xyz_to_dist_torch
...
class ConstantKernel(Kernel):
    ...
    def __init__(
        ...
        self.register_buffer(
            name="raw_constant",
            parameter=torch.nn.Parameter(torch.zeros(*self.batch_shape, 1, num_dist)),
        )
    ...
    def weighted_constant(self, dist):
        ...
        weighted_constant = torch.sum(self.constant * weights)
    ...
    def forward(
        ...
        """
        w_constant = self.weighted_constant(xyz_to_dist_torch(x1))
        ...
        constant = w_constant.to(dtype=dtype, device=x1.device)

        if not diag:
            w_constant = constant.unsqueeze(-1)

        if last_dim_is_batch:
            w_constant = constant.unsqueeze(-1)

        return w_constant.expand(shape)