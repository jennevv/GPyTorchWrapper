...
def sample_fitted_transformer(sample_dataframe):
    ...
    )  # .values is used to get the numpy array, so no column names are used in the fit
...
def test_transform_data_unfitted_2(sample_dataframe):
    ...
    df, transformer = transform_data(
        sample_dataframe, unfitted_transformer, columns=["a"]
    )

    assert np.allclose(
        df["a"],
        pd.DataFrame(StandardScaler().fit_transform(sample_dataframe[["a"]])).squeeze(),
    )
...
def test_transform_data_unfitted_3(sample_dataframe):
    ...
    df, transformer = transform_data(
        sample_dataframe, unfitted_transformer, columns=[0]
    )
...
def test_transform_data_unfitted_4(sample_dataframe):
    ...
    df, transformer = transform_data(
        sample_dataframe, unfitted_transformer, columns=[0, 1]
    )

    assert np.allclose(
        df[["a", "b"]],
        pd.DataFrame(StandardScaler().fit_transform(sample_dataframe[["a", "b"]])),
    )
