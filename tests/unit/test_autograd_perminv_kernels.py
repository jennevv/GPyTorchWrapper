...
class RBFModel(gpytorch.models.ExactGP):
    def __init__(self, train_x, train_y, likelihood):
        super().__init__(train_x, train_y, likelihood)
        self.mean_module = gpytorch.means.ZeroMean()
        self.covar_module = gpytorch.kernels.ScaleKernel(RBFKernelPermInv(n_atoms=3, idx_equiv_atoms=[[0,1]], ard=True))
    def forward(self, x):
        mean_x = self.mean_module(x)
        covar_x = self.covar_module(x)
...
class LinearxRBFModel(gpytorch.models.ExactGP):
    def __init__(self, train_x, train_y, likelihood):
        super().__init__(train_x, train_y, likelihood)
        self.mean_module = gpytorch.means.ZeroMean()
        self.covar_module = gpytorch.kernels.ScaleKernel(LinearxRBFKernelPermInv(n_atoms=3, idx_equiv_atoms=[[0,1]], ard=True))
    def forward(self, x):
        mean_x = self.mean_module(x)
        covar_x = self.covar_module(x)
...
class LinearxMaternModel(gpytorch.models.ExactGP):
    def __init__(self, train_x, train_y, likelihood):
        super().__init__(train_x, train_y, likelihood)
        self.mean_module = gpytorch.means.ZeroMean()
        self.covar_module = gpytorch.kernels.ScaleKernel(LinearxMaternKernelPermInv(n_atoms=3, idx_equiv_atoms=[[0,1]], ard=True))
    def forward(self, x):
        mean_x = self.mean_module(x)
        covar_x = self.covar_module(x)
...
class PolyxMaternModel(gpytorch.models.ExactGP):
    def __init__(self, train_x, train_y, likelihood):
        super().__init__(train_x, train_y, likelihood)
        self.mean_module = gpytorch.means.ZeroMean()
        self.covar_module = gpytorch.kernels.ScaleKernel(PolyxMaternKernelPermInv(n_atoms=3, idx_equiv_atoms=[[0,1]], ard=True, power=1))
    def forward(self, x):
        mean_x = self.mean_module(x)
        covar_x = self.covar_module(x)
...
def test_matern_model(sample_training_data, sample_test_data):
    ...
    assert torch.allclose(jac.squeeze(), numerical_gradient(test_x.clone().detach(), mean_f))
