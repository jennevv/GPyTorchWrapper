...
def run_script_help():
    result = subprocess.run(["python", "training_gpytorch.py", "--help"], check=True, capture_output=True, text=True)
    return result
...
def run_script_single_output():
    subprocess.run([
        ...
        "-f", "csv",
        "-c", "./tests/end2end/single_output_config.yml",
        "-o", "test_model_single_output",
        "-d", "./test_single_model"
    ], check=True)
